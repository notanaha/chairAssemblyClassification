...
Notes:
- Now uses AsyncAzureOpenAI via chairAssemblyStep.run_async for true async concurrency.
...
# Local import
from utils import chairAssemblyStepAsync as chairAssemblyStep
...
async def _run_one(step: str, images: Sequence[str], verbose: bool, sem: asyncio.Semaphore) -> Tuple[str, str, float]:
    start = time.time()
    async with sem:
        result = await chairAssemblyStep.run_async(step, list(images), verbose)
